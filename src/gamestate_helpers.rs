...
pub fn is_won(gamestate: u128) -> Option<PlayerColor> {
    ...
    let blue_winning_gamestates: Vec<u128> = vec![85, 1392640, 22817013760, 373833953443840, 
    6124895493223874560, 100350287760979960791040, 341, 5586944, 91536490496, 1499733860286464, 
    24571639566933426176, 402581742664637254467584, 1365, 22364160, 366414397440, 6003333487656960, 
    98358615861771632640, 1611507562279266429173760, 5461, 89473024, 1465926025216, 
    24017731997138944, 393506521041124458496, 6447210840737783127998464, 4398314962945, 
    17593259851780, 70373039407120, 281492157628480, 1125968630513920, 4503874522055680, 
    18015498088222720, 72061992352890880, 288247969411563520, 1152991877646254080, 
    4611967510585016320, 18447870042340065280, 73791480169360261120, 295165920677441044480, 
    1180663682709764177920, 4722654730839056711680, 18890618923356226846720, 
    75562475693424907386880, 302249902773699629547520, 1208999611094798518190080, 
    4835998444379194072760320, 281479271743489, 1125917086973956, 4503668347895824, 
    18014673391583296, 4611756388245323776, 18447025552981295104, 73788102211925180416, 
    295152408847700721664, 75559016665011384745984, 302236066660045538983936, 
    1208944266640182155935744, 4835777066560728623742976, 4399120515136, 17596482060544, 
    70385928242176, 281543712968704, 72075190519988224, 288300762079952896, 1153203048319811584, 
    4612812193279246336, 1180879921479487062016, 4723519685917948248064, 18894078743671792992256, 
    75576314974687171969024];

    //Vector of possible winning combinations for red
    let red_winning_gamestates: Vec<u128> = vec![170, 2785280, 45634027520, 747667906887680, 
    12249790986447749120, 200700575521959921582080, 682, 11173888, 183072980992, 2999467720572928, 
    49143279133866852352, 805163485329274508935168, 2730, 44728320, 732828794880, 12006666975313920, 
    196717231723543265280, 3223015124558532858347520, 10922, 178946048, 2931852050432, 
    48035463994277888, 787013042082248916992, 12894421681475566255996928, 8796629925890, 
    35186519703560, 140746078814240, 562984315256960, 2251937261027840, 9007749044111360, 
    36030996176445440, 144123984705781760, 576495938823127040, 2305983755292508160, 
    9223935021170032640, 36895740084680130560, 147582960338720522240, 590331841354882088960, 
    2361327365419528355840, 9445309461678113423360, 37781237846712453693440, 
    151124951386849814773760, 604499805547399259095040, 2417999222189597036380160, 
    9671996888758388145520640, 562958543486978, 2251834173947912, 9007336695791648, 
    36029346783166592, 9223512776490647552, 36894051105962590208, 147576204423850360832, 
    590304817695401443328, 151118033330022769491968, 604472133320091077967872, 
    2417888533280364311871488, 9671554133121457247485952, 8798241030272, 35192964121088, 
    140771856484352, 563087425937408, 144150381039976448, 576601524159905792, 2306406096639623168, 
    9225624386558492672, 2361759842958974124032, 9447039371835896496128, 37788157487343585984512, 
...
pub fn is_allowed_move(gamestate: u128, move_to_check: u128, turn_number: usize) -> bool {
    ...
    match whos_turn_is_it_turn_number(turn_number) {
        ...
        PlayerColor::Blue => if move_to_check & (6447604371278022265099605) != move_to_check {return false;}

        // Case where it Red's turn and constant is sum of all powers of 2 with odd exponents
        PlayerColor::Red => if move_to_check & 12895208742556044530199210 != move_to_check {return false;}
    }
    ...
    if move_to_check >= BASE.pow(70) {return true;}
...
pub fn turn_column_to_encoded_gamestate(gamestate: u128, column: u32, color: &PlayerColor) -> Option<u128> {
...
mod tests {
    ...
    fn is_allowed_move_given_not_allowed_move_return_false() {
        ...
        assert_eq!(is_allowed_move(BASE.pow(31) + BASE.pow(28), BASE.pow(31), 3), false);
    }
    ...
    fn turn_column_to_encoded_gamestate_given_correct_tuples_blue_return_encoded_move() {
        assert_eq!(turn_column_to_encoded_gamestate(0, 7, &PlayerColor::Blue), Some(BASE.pow(82)));
        assert_eq!(turn_column_to_encoded_gamestate(0, 6, &PlayerColor::Blue), Some(BASE.pow(80)));
    }

    #[test]
    fn turn_column_to_encoded_gamestate_given_correct_tuples_red_return_encoded_move() {
        assert_eq!(turn_column_to_encoded_gamestate(0, 6, &PlayerColor::Red), Some(BASE.pow(81)));
    }
