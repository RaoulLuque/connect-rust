...
pub struct Engine {
    ...
    // Evaluation +i32::max-x stands for the current player winning in x turns 
    // Evaluation -i32::max+x stands for the opposing player winning in x turns
...
impl Engine {
    ...
    pub fn make_move(&self, gamestate: u32) -> u32 {
        ...
        for successor in self.gamestate_graph.out_neighbours(&gamestate) {
            ...
            if let (Ok(s), Ok(b)) = (self.gamestate_graph.get_label(successor).unwrap().parse(),
                                  self.gamestate_graph.get_label(&best_succesor).unwrap().parse()) {
                if s > b {
                    best_succesor = *successor;
                } else {
